...
def generate_map():
    ...
    for i in range(1, 81):
        ...
        z = i * 15
        y = 5 + (i % 5) * 3
        traps.append(create_obstacle(
            [round(x, 2), round(y, 2), round(z, 2)],
            [4, 2, 4],
            f"RGB({i*3%255}, {i*7%255}, {i*11%255})"
        ))
    return traps

if __name__ == "__main__":
    print("=" * 50)
    print("بدء إنشاء خريطة الباركور...")
    print("=" * 50)
    ...
    print(f"✅ تم حفظ الخريطة في ملف: {file_path}")
    print("\n📌 أول خدعة:")
    print(map_data[0])
    print("\n📌 آخر خدعة:")
    print(map_data[-1])
    ...
    if os.path.exists(file_path):
        print(f"\n✅ الملف موجود: {file_path} (حجمه: {os.path.getsize(file_path)} بايت)")
    else:
        print(f"\n❌ الملف غير موجود!")
